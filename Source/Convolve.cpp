...
void Convolve::blur(Read_in& read){
    ...
    float a=0.0;
    for(int i=2; i<read.row-2; i++){
        for(int j=2; j<read.column-2; j++){
            for(int k=0; k<5; k++){
                for(int l=0; l<5; l++){
                    if(level==1){
    ...
    read.image_name=read.image_name+"_blur"+to_string(level)+".pgm";
}

void Convolve::edge(Read_in& read){
    ...
    float G=0;
    float G_max=0;
    for(int i=1; i<read.row-1; i++){
        for(int j=1; j<read.column-1; j++){
            for(int k=0; k<3; k++){
                for(int l=0; l<3; l++){
            ...
            read.vect[i][j]=(int)((G/G_max)*read.max_intensity);  //Adjusting values between 0 and read.max_intensity.
            a1=0.0;
            a2=0.0;
            G=0.0;
        }
    }
    read.image_name=read.image_name+"_edges.pgm";
}

void Convolve::sharpen(Read_in& read){

    vector <vector<int> > vect_temp1(read.row, vector<int>(read.column, 0));
    vector <vector<int> > vect_temp2(read.row, vector<int>(read.column, 0));
    vector <vector<int> > vect_edge(read.row, vector<int>(read.column, 0));

    for(int i=0; i<read.row; i++){
        for(int j=0; j<read.column; j++){
            vect_temp1[i][j]=read.vect[i][j];
            vect_temp2[i][j]=read.vect[i][j];
        }
    }
    ...
    for(int i=1; i<read.row-1; i++){
        for(int j=1; j<read.column-1; j++){
            ...
            vect_edge[i][j]=(int)((G/G_max)*read.max_intensity);
            a1=0.0;
            a2=0.0;
            G=0.0;
        }
    ...
    for(int i=1; i<read.row-1; i++){
        for(int j=1; j<read.column-1; j++){
            if(vect_edge[i][j]>150 && read.vect[i][j]*1.2<read.max_intensity){
                vect_temp2[i][j]=vect_temp2[i][j]*1.2;
                for(int k=0; k<3; k++){
                    for(int l=0; l<3; l++){
                        if(read.vect[i+k-1][j+l-1]<0.9*read.vect[i][j]){
                            vect_temp2[i+k-1][j+l-1]=0.8*vect_temp2[i+k-1][j+l-1];
                        }
                    }
                }
            }
            else if(vect_edge[i][j]>150 && read.vect[i][j]*1.2>read.max_intensity){
                vect_temp2[i][j]=read.max_intensity;
                for(int k=0; k<3; k++){
                    for(int l=0; l<3; l++){
                        if(read.vect[i+k-1][j+l-1]<0.9*read.vect[i][j]){
                            vect_temp2[i+k-1][j+l-1]=0.8*vect_temp2[i+k-1][j+l-1];
    ...
    read.vect.swap(vect_temp2);

    read.image_name=read.image_name+"_sharpened.pgm";
}
